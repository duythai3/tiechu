...
//
gboolean a_pinyin_editor_is_signal_letter(guint keyval){
    switch(keyval){
        case IBUS_l:
        case IBUS_z:
        case IBUS_x:
...
static gboolean process_pinyin(guint keyval){
    ...
    a_logger_log("ascii a_clutter: %s", ascii_clutter);
	if (!ascii_clutter) {
		return FALSE;
	}
	...
	//
	const gchar *pinyin_clutter =a_ltelex_table_convert_to_pinyin(down_asscii_clutter);
...
/////////////////////////
gboolean a_pinyin_editor_process(IBusAbacusEngine* abacus, guint keyval, guint keycode, guint modifiers){
