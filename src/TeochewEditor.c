...
static gboolean process_teochew(guint keyval){
	...
	//
    const gchar *teochew_clutter =etelex_table_convert_to_pinyin(down_asscii_clutter);
    logger_log("teochew_clutter:%s", teochew_clutter);
    ...
    preedit_replace(start_pos, clutter_len, unichar_teochew_clutter, teochew_len);
	g_free(ascii_clutter);
...
gboolean teochew_editor_is_preedit_containing_tone() {
    ...
    for (gint i = 0; i < preedit_len; i++) {
        guint c = (uint) preedit_get_char(i);
