...
static gboolean process_telex(guint keyval){
    ...
    preedit_replace(start_pos, clutter_len, vn_clutter, vn_len);
	g_free(telex_clutter);
	g_free(vn_clutter);
	return TRUE;
}
...
/////////////////////////
gboolean telex_process(IBusTiechuEngine *tiechu, guint keyval, guint keycode, guint modifiers){
