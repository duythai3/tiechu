...
gboolean a_vietnamese_mode_process_key_event(IBusAbacusEngine* abacus, guint keyval, guint keycode, guint modifiers) {
    ...
    guint len =a_preedit_get_length();
    ...
    // process letter keys
    if (is_alpha(keyval)) {
        if(a_telex_2_process(abacus, keyval, keycode, modifiers)){
            a_helper_update_preedit(abacus);
        }else{
    ...
    if(is_kp_digit(keyval)){
        a_preedit_insert(convert_kp_digit_to_digit(keyval));
        a_helper_update_preedit(abacus);
        return TRUE;
    }
