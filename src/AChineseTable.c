...
// initialize
gboolean a_chinese_table_init(){
	//
	_char_expand_table_init();
    a_logger_log("Chinese table opened");
	return TRUE;
}

// destroy
void a_chinese_table_destroy(){
	//
	_char_expand_table_destroy();
    a_logger_log("Chinese table destroyed");
}

gboolean a_chinese_table_find(){
	//
   ...
   if(preedit_len <= 7)
	  ...
	  target_word_len = 7;
   len = g_sprintf(_buff, "select distinct _char, _frequency from %s where", "chinese");
   index = len;
   ...
   for(gint i = 0; i < target_word_len; i++){
       ...
       //
       if(i == 0){
            len = g_sprintf(_buff + index, " _p_%d=%d", i+1, ch);
            index = index + len;
       }else{
            len = g_sprintf(_buff + index, " and _p_%d=%d", i+1, ch);
            index = index + len;
       }
...
static int _add_word(void *NotUsed, int argc, char **argv, char **azColName){
	...
	if(chinese_char){
        a_candidate_array_add(g_strdup(chinese_char));
    }
    return 0;
}
...
// increase frequency
void a_chinese_table_increase_frequency(const gchar *chinese_char){
	//
	...
	//
    gint len = g_sprintf(_buff, "update chinese set _frequency=_frequency+1 where _char='%s'", chinese_char);
	_buff[len] = 0;
    a_logger_log(_buff);
	gchar *errmsg = NULL;
